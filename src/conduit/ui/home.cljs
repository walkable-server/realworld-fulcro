  ...
  (dom/nav :.navbar.navbar-light
    (dom/div :.container
      ...
      (dom/ul :.nav.navbar-nav.pull-xs-right
        (dom/li :.nav-item
          (dom/a :.nav-link
            {:className (when (or (#{[:component/id :global-feed]
                                     [:component/id :personal-feed]}
                                    current-route)
                                (= :articles/by-tag (first current-route)))
                          "active")
             :href      "/home"}
            "Home"))
        (when logged-in?
          (dom/li :.nav-item
            (dom/a :.nav-link
              {:className (when (= current-route [:component/id :new])
                            "active")
               :href      "/new"}
              (dom/i :.ion-compose)
              "New Post")))
        (when logged-in?
          (dom/li :.nav-item
            (dom/a :.nav-link
              {:className (when (= current-route [:component/id :settings])
                            "active")
               :href "/settings"}
              (dom/i :.ion-gear-a)
              "Settings")))
        (when-not logged-in?
          (dom/li :.nav-item
            (dom/a :.nav-link
              {:className (when (= current-route [:component/id :login])
                            "active")
               :href      "/login"}
              "Login")))
        ...
        (when-not logged-in?
          (dom/li :.nav-item
            (dom/a :.nav-link
              {:className (when (= current-route [:component/id :sign-up])
                            "active")
               :href      "/sign-up"}
              "Sign up")))
